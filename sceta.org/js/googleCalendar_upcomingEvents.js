$(document).ready(function() {
	...
	var container = jQuery('div#gcal_upcomingEvents');
	...
	var calendarUrl = 'https://www.googleapis.com/calendar/v3/calendars/' + calendarId 
		+ '/events?fields=items(end%2ChtmlLink%2Clocation%2CoriginalStartTime%2Cstart%2Csummary)'
		+ '&maxResults=3&orderBy=startTime&singleEvents=true&timeMin=' + todayFormatted 
		+ '&key=' + apiKey;
	...
	function feedCallback(data) {
	    ...
	    for (i in data['items']) {
			...
			var locationUrl = '<a href="' + mapsUrl + '" target="_blank">map</a>';
		...
		// If there are no upcoming events, then display that message.
		if (events == '') {
			container.last().before('There are no upcoming events.');
		}

		// Otherwise, display the upcoming events.
		else {
			container.last().before(events);
		}
	}
